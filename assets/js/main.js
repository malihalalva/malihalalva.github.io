...
//Ind Skills in Mechanical Skills Section
const skillsItem = document.getElementsByClassName('skills__item'),
      skillsName = document.querySelectorAll('.indSkill__Header')

function toggleIndSkills(){
    let itemClass = this.parentNode.className

    for(i = 0; i < skillsItem.length; i++){
        skillsItem[i].className = 'skills__item skillsInd__close';
    }

    if(itemClass === 'skills__item skillsInd__close'){
        this.parentNode.className = 'skills__item skillsInd__open';
    }

}

skillsName.forEach((el)  =>{
    el.addEventListener('click', toggleIndSkills)
...
function toggleSkills(clickedArrow) {
    const skillsContent = clickedArrow.closest('.skills__content');
    skillsContent.classList.toggle('skills__open');
    skillsContent.classList.toggle('skills__close');
    clickedArrow.classList.toggle('skills__arrow--rotate');
}

function toggleDescription(event, clickedArrow) {
    console.log(clickedArrow.classList)
    event.stopPropagation(); // Prevent event from bubbling up to the parent

    const skillItem = clickedArrow.closest('.skills__item');
    const description = skillItem.querySelector('.skills__description');
    const link = skillItem.querySelector('.skills__link');

    // Toggle visibility
    description.style.display = description.style.display === 'none' ? 'block' : 'none';
    link.style.display = link.style.display === 'none' ? 'inline-block' : 'none';

    // Ensure this toggles the class on the arrow icon itself
    clickedArrow.classList.toggle('skills__arrow--rotate');
}
*/
...
var btnDescClick = false;

function ShowDesc() {
    if (!btnDescClick) {
        for(i = 0; i < skillsItem.length; i++){
            skillsItem[i].className = 'skills__item skillsInd__open';
        }

        btnDescClick = true;

        document.querySelector('#btnExpandCollapse').value = 'Collapse All';
    }
    else {
        for(i = 0; i < skillsItem.length; i++){
            skillsItem[i].className = 'skills__item skillsInd__close';
        ...
        document.querySelector('#btnExpandCollapse').value = 'Expand All';
    }
}
...
//         let isAnyOpen = Array.from(allSkillsContents).some(content => content.classList.contains('skills__open'));
...
//                 arrow.classList.remove('skills__arrow--rotate');
//             } else {
//                 content.classList.add('skills__open');
//                 content.classList.remove('skills__close');
//                 list.style.display = 'block';
//                 arrow.classList.add('skills__arrow--rotate');
//             }
//         });

//         expandAllBtn.textContent = isAnyOpen ? 'Expand All' : 'Collapse All';
//         isAnyOpen = !isAnyOpen;
//     });
// });
